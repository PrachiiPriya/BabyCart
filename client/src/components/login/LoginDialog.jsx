...
export default function LoginDialog({ open, setopen }) {
  ...
  const signupUser = useCallback(async () => {
    try {
      const response = await axios.post('http://localhost:8000/api/loginSignup/signup', signup);
      if (response.status === 200) {
        handleClose();
        setContextAccount(signup.username);
      }
    } catch (error) {
  ...
  const loginUser = useCallback(async () => {
    try {
      const response = await axios.post('http://localhost:8000/api/loginSignup/login', login);
      if (response.status === 200) {
        handleClose();
        setContextAccount(response.data.data.username);
      } else {
